...
if(config.footerTemplate){self.addTemplateSafe(config.footerTemplate,kg.templates.defaultFooterTemplate);}};this.getTemplateText=function(tmplId){if(!self.templateExists(tmplId)){return"";}else{var el=document.getElementById(tmplId);return el.text;}};}());﻿kg.Dimension=function(options){this.innerHeight=null;this.innerWidth=null;this.outerHeight=null;this.outerWidth=null;this.widthDiff=null;this.heightDiff=null;this.autoFitHeight=false;this.autoFitWidth=false;$.extend(this,options);};﻿kg.Cell=function(col){this.data='';this.column=col;this.row=null;};﻿kg.Column=function(colDef){var self=this,wIsOb=ko.isObservable(colDef.width),minWIsOB=ko.isObservable(colDef.minWidth),maxWIsOB=ko.isObservable(colDef.maxWidth);this.width=wIsOb?colDef.width:ko.observable(0);this.minWidth=minWIsOB?colDef.minWidth:(!colDef.minWidth?ko.observable(50):ko.observable(colDef.minWidth));this.maxWidth=maxWIsOB?colDef.maxWidth:(!colDef.maxWidth?ko.observable(9000):ko.observable(colDef.maxWidth));this.field=colDef.field;if(colDef.displayName===undefined||colDef.displayName===null){colDef.displayName=colDef.field;}
...
this.hasHeaderTemplate=(this.headerTemplate?true:false);if(!colDef.width||colDef.width()=="*"){colDef.width=this.displayName.length*kg.domUtility.letterW;colDef.width+=30;self.width(colDef.width);}else if(!wIsOb){self.width(colDef.width);}};﻿kg.ColumnCollection=function(){var obs=ko.observableArray([]);ko.utils.extend(obs,kg.ColumnCollection.fn);return obs;};kg.ColumnCollection.fn={};﻿
...
row.cells(cells);return row;};};﻿kg.HeaderCell=function(col){var self=this;this.colIndex=col.colIndex;this.displayName=col.displayName;this.field=col.field;this.column=col;this.headerClass=col.headerClass;this.headerTemplate=col.headerTemplate;this.hasHeaderTemplate=col.hasHeaderTemplate;this.allowSort=ko.observable(col.allowSort);this.allowFilter=col.allowFilter;this.allowResize=ko.observable(col.allowResize);this.width=col.width;this.minWidth=col.minWidth;this.maxWidth=col.maxWidth;this.filter=ko.computed({read:function(){return self.column.filter();},write:function(val){self.column.filter(val);}});this.filterVisible=ko.observable(false);this._filterVisible=ko.computed({read:function(){return self.allowFilter;},write:function(val){self.filterVisible(val);}});this.sortAscVisible=ko.computed(function(){return self.column.sortDirection()==="asc";});this.sortDescVisible=ko.computed(function(){return self.column.sortDirection()==="desc";});this.noSortVisible=ko.computed(function(){var sortDir=self.column.sortDirection();return sortDir!=="asc"&&sortDir!=="desc";});this.sort=function(){if(!self.allowSort()){return;}
var dir=self.column.sortDirection()==="asc"?"desc":"asc";self.column.sortDirection(dir);};this.filterHasFocus=ko.observable(false);﻿ this.startMousePosition=0;﻿ this.startMousePosition=0;this.origWidth=0;﻿
this.gripOnMouseUp=function(){document.onmousemove=null;document.onmouseup=null;document.body.style.cursor='default';return false;};this.onMouseMove=function(event){var diff=event.clientX-self.startMousePosition;var newWidth=diff+self.origWidth;self.width(newWidth<self.minWidth()?self.minWidth():(newWidth>self.maxWidth()?self.maxWidth():newWidth));return false;﻿};﻿
this.gripOnMouseDown=function(event){self.startMousePosition=event.clientX;self.origWidth=self.width();﻿ document.onmousemove=self.onMouseMove;﻿ document.onmouseup=self.gripOnMouseUp;document.body.style.cursor='col-resize';event.target.parentElement.style.cursor='col-resize';return false;};};﻿kg.HeaderRow=function(){this.headerCells=[];this.height;this.headerCellMap={};this.filterVisible=ko.observable(false);};﻿kg.RowManager=function(grid){var self=this,prevMaxRows=0,prevMinRows=0,currentPage=grid.config.currentPage,pageSize=grid.config.pageSize,prevRenderedRange=new kg.Range(0,1),prevViewableRange=new kg.Range(0,1),internalRenderedRange=ko.observable(prevRenderedRange);this.dataChanged=true;this.rowCache=[];this.dataSource=grid.finalData;this.dataSource.subscribe(function(){self.dataChanged=true;self.rowCache=[];});this.minViewportRows=grid.minRowsToRender;this.excessRows=8;this.rowHeight=grid.config.rowHeight;this.cellFactory=new kg.CellFactory(grid.columns());this.viewableRange=ko.observable(prevViewableRange);this.renderedRange=ko.observable(prevRenderedRange);this.rows=ko.observableArray([]);this.rowSubscriptions={};this.buildRowFromEntity=function(entity,rowIndex,pagingOffset){var row=self.rowCache[rowIndex];if(!row){row=new kg.Row(entity,grid.config,grid.selectionManager);row.rowIndex=rowIndex+1;row.rowDisplayIndex=row.rowIndex+pagingOffset;row.offsetTop=self.rowHeight*rowIndex;self.cellFactory.buildRowCells(row);self.rowCache[rowIndex]=row;}
return row;};this.renderedRange.subscribe(function(rg){var rowArr=[],row,pagingOffset=(pageSize()*(currentPage()-1)),dataArr=self.dataSource().slice(rg.bottomRow,rg.topRow);utils.forEach(dataArr,function(item,i){row=self.buildRowFromEntity(item,rg.bottomRow+i,pagingOffset);item.myRowEntity=row;rowArr.push(row);row=null;});self.rows(rowArr);});this.calcRenderedRange=function(){var rg=self.viewableRange(),minRows=self.minViewportRows(),maxRows=self.dataSource().length,isDif=false,newRg;if(rg){isDif=(rg.bottomRow!==prevViewableRange.bottomRow||rg.topRow!==prevViewableRange.topRow||self.dataChanged)
...
if(isDif){rg.topRow=rg.bottomRow+minRows;prevViewableRange=rg;newRg=new kg.Range(rg.bottomRow,rg.topRow);newRg.bottomRow=Math.max(0,rg.bottomRow-self.excessRows);newRg.topRow=Math.min(maxRows,rg.topRow+self.excessRows);prevMaxRows=maxRows;prevMinRows=minRows;if(prevRenderedRange.topRow!==newRg.topRow||prevRenderedRange.bottomRow!==newRg.bottomRow||self.dataChanged){self.dataChanged=false;prevRenderedRange=newRg;self.renderedRange(newRg);}}}else{self.renderedRange(new kg.Range(0,0));}};self.viewableRange.subscribe(self.calcRenderedRange);self.minViewportRows.subscribe(self.calcRenderedRange);self.dataSource.subscribe(self.calcRenderedRange);};﻿kg.Footer=function(grid){var self=this;this.maxRows;if(grid.config.totalServerItems()!==null&&grid.config.totalServerItems()!==undefined){this.maxRows=grid.config.totalServerItems;}else{this.maxRows=grid.maxRows;}
...
if(dim.innerHeight&&dim.innerWidth){$container.height(dim.innerHeight);$container.width(dim.innerWidth);return;};if(oldHt!==dim.outerHeight||oldWdth!==dim.outerWidth){$container.height(dim.outerHeight).width(dim.outerWidth);oldHt=$container.outerHeight();oldWdth=$container.outerWidth();dim.heightDiff=oldHt-$container.height();dim.widthDiff=oldWdth-$container.width();$container.height(dim.outerHeight-dim.heightDiff);$container.width(dim.outerWidth-dim.widthDiff);}}}};}());ko.bindingHandlers['mouseEvents']=(function(){return{'init':function(element,valueAccessor,allBindingsAccessor,viewModel,bindingContext){var eFuncs=valueAccessor();if(eFuncs.mouseDown){$(element).mousedown(eFuncs.mouseDown);}},'update':function(element,valueAccessor,allBindingsAccessor,viewModel,bindingContext){}};}());}(window));