(function(window,undefined){ko.kgMoveSelection=function(sender,evt){var offset,charCode=(evt.which)?evt.which:event.keyCode;switch(charCode){case 38:offset=-1;break;case 40:offset=1;break;default:return true;}
...
if(isMulti){selectedItems=self.selectedItems();itemsToRemove=[];ko.utils.arrayForEach(selectedItems,function(item){if(ko.utils.arrayIndexOf(items,item)<0){itemsToRemove.push(item);}});if(itemsToRemove.length>0){self.selectedItems.removeAll(itemsToRemove);}}else{selectedItem=self.selectedItem();if(selectedItem&&ko.utils.arrayIndexOf(items,selectedItem)<0){self.selectedItem(items[0]?items[0]:null);}}});};﻿kg.gridManager=(new function(){var self=this,elementGridKey='__koGrid__';this.gridCache={};this.storeGrid=function(element,grid){self.gridCache[grid.gridId]=grid;element[elementGridKey]=grid.gridId;if(element.tabIndex==-1){element.tabIndex=self.getIndexOfCache(grid.gridId);}};this.removeGrid=function(gridId){delete self.gridCache[gridId];};this.getGrid=function(element){var grid;if(element[elementGridKey]){grid=self.gridCache[element[elementGridKey]];}
