...
self.filterInfo(info);};};initPhase=1;};﻿﻿kg.SortManager=function(options){var self=this,colSortFnCache={},dateRE=/^(\d\d?)[\/\.-](\d\d?)[\/\.-]((\d\d)?\d\d)$/,ASC="asc",DESC="desc",prevSortInfo={},dataSource=options.data,initPhase=0,internalSortedData=ko.observableArray([]);this.isEmpty=function(val){return(val===null||val===undefined||val==='');};this.sortInfo=options.sortInfo||ko.observable();this.sortedData=ko.computed(function(){var sortData=internalSortedData();if(initPhase>0){return sortData;}else{return dataSource();}});this.guessSortFn=function(item){var sortFn,itemStr,itemType,dateParts,month,day;if(item===undefined||item===null||item===''){return null;}
...
if(keep){self.selectedItem(changedEntity);}}else{len=currentItems().length;if(changedEntity&&changedEntity[KEY]){keep=changedEntity[KEY]();}
