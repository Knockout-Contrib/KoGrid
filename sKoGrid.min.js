...
kg.SelectionManager=function(options){var self=this,isMulti=options.isMultiSelect,dataSource=options.data,KEY='__kg_selected__',maxRows=ko.computed(function(){return dataSource().length;});this.selectedItem=options.selectedItem;this.selectedItems=options.selectedItems;this.selectedIndex=options.selectedIndex;this.keepLastSelectedAround=options.keepLastSelectedAround;this.selectedItemCount=ko.computed(function(){var single=self.selectedItem(),arr=self.selectedItems();if(!isMulti){return(single!==null&&single!==undefined)?1:0;}else{return arr.length;}});this.selectedItem.subscribe(function(currentEntity){if(!isMulti){if(currentEntity&&currentEntity[KEY]){currentEntity[KEY](false);}}},self,"beforeChange");this.selectedItem.subscribe(function(entity){if(entity&&!entity[KEY]){entity[KEY]=ko.observable(true);}else if(entity){entity[KEY](true);}});this.selectedItems.subscribe(function(newItems){var data=dataSource();if(!newItems){newItems=[];}
...
if(keep){self.selectedItem(changedEntity);}else{if(self.keepLastSelectedAround){changedEntity[KEY](true);}else{self.selectedItem(undefined);}}}else{len=currentItems().length;if(changedEntity&&changedEntity[KEY]){keep=changedEntity[KEY]();}
