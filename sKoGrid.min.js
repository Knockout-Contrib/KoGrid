...
if(self.initPhase>0){if(!filterIsOpen()&&!isSorting){self.refreshDomSizes();kg.cssBuilder.buildStyles(self);if(self.initPhase>0&&self.$root){self.$root.show();}}}});this.buildColumnDefsFromData=function(){var item;if(!self.data()||!self.data()[0]){throw'If auto-generating columns, "data" cannot be of null or undefined type!';}
...
ko.bindingHandlers['koGrid']=(function(){var makeNewValueAccessor=function(grid){return function(){return{name:GRID_TEMPLATE,data:grid};};};return{'init':function(element,valueAccessor,allBindingsAccessor,viewModel,bindingContext){var grid,options=valueAccessor(),$element=$(element);var grid=kg.gridManager.getGrid(element);if(!grid){grid=new kg.KoGrid(options);}
var gridId=grid.gridId.toString();grid.config.columnDefs.subscribe(function(newColumns){var oldgrid=kg.gridManager.getGrid(element);var oldgridId=oldgrid.gridId.toString();$(element).empty();$(element).removeClass("kgGrid").removeClass("ui-widget").removeClass(gridId);kg.gridManager.removeGrid(gridId);ko.applyBindings(bindingContext,element);});kg.gridManager.storeGrid(element,grid);kg.domUtility.measureGrid($element,grid,true);$element.hide();$(element).addClass("kgGrid").addClass("ui-widget").addClass(grid.gridId.toString());var body=document.getElementsByTagName("body")[0];var bodyAttrib=body.getAttribute("data-bind");if(bodyAttrib==null){$(element).removeClass(gridId);body.setAttribute("data-bind","event: { keydown: ko.kgMoveSelection }");ko.applyBindings(bindingContext.$parent,body);}
