...
kg.SelectionManager=function(options,rowManager){var self=this,dataSource=options.data,KEY='__kg_selected__',maxRows=ko.computed(function(){return dataSource().length;});this.selectedItems=options.selectedItems;this.selectedIndex=options.selectedIndex;this.lastClickedRow=options.lastClickedRow;this.changeSelection=function(rowItem,clickEvent){if(clickEvent.shiftKey){document.getSelection().removeAllRanges();if(self.lastClickedRow()){var thisIndx=rowManager.rowCache.indexOf(rowItem);var prevIndex=rowManager.rowCache.indexOf(self.lastClickedRow());if(thisIndx<prevIndex){thisIndx=thisIndx^prevIndex;prevIndex=thisIndx^prevIndex;thisIndx=thisIndx^prevIndex;}
for(;prevIndex<=thisIndx;prevIndex++){rowManager.rowCache[prevIndex].selected(true);if(self.selectedItems.indexOf(rowManager.rowCache[prevIndex].entity())===-1){self.selectedItems.push(rowManager.rowCache[prevIndex].entity());}}}}else if(clickEvent.ctrlKey){self.toggle(rowItem);}else{utils.forEach(self.selectedItems(),function(item){item.myRowEntity.selected(false);});self.selectedItems.removeAll();self.toggle(rowItem);}
