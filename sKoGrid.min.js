...
if(grid!=null&&grid!=undefined){if(grid.config.selectedItems()!=undefined){var items=grid.finalData();var n=items.length;var index=items.indexOf(grid.config.lastClickedRow().entity())+offset;if(index>=0&&index<n){utils.forEach(grid.config.selectedItems(),function(itm){itm.myRowEntity.selected(false);});grid.config.selectedItems.removeAll();var selected=items[index];grid.config.selectedItems.push(selected);grid.config.lastClickedRow(selected.myRowEntity);var itemtoView=document.getElementsByClassName("kgSelected");if(!Element.prototype.scrollIntoViewIfNeeded){itemtoView[0].scrollIntoView(false);}else{itemtoView[0].scrollIntoViewIfNeeded();}}}}};﻿
...
kg.Row=function(entity,config,rowCache){var self=this,KEY='__kg_selected__',canSelectRows=config.canSelectRows;this.rows=rowCache;this.selectedItems=config.selectedItems;this.entity=ko.isObservable(entity)?entity:ko.observable(entity);if(this.entity()['__kg_selected__']===undefined){this.entity()['__kg_selected__']=ko.observable(false);}
...
kg.SelectionManager=function(options){var self=this,dataSource=options.data,KEY='__kg_selected__',maxRows=ko.computed(function(){return dataSource().length;});this.selectedItems=options.selectedItems;this.selectedIndex=options.selectedIndex;this.keepLastSelectedAround=options.keepLastSelectedAround;this.selectedItemCount=ko.computed(function(){return self.selectedItems().length;});this.selectedItems.subscribe(function(newItems){var data=dataSource();if(!newItems){newItems=[];}
