...
this.displayName=colDef.displayName;this.colIndex=0;this.isVisible=ko.observable(false);this.width=ko.observable();if(colDef.sortable===undefined||colDef.sortable===null){colDef.sortable=true;}
this.allowSort=colDef.sortable;this.sortDirection=ko.observable("");this.filter=ko.observable();this.cellTemplate=colDef.cellTemplate;this.hasCellTemplate=(this.cellTemplate?true:false);this.cellClass=colDef.cellClass;this.headerClass=colDef.headerClass;this.headerTemplate=colDef.headerTemplate
...
kg.Row=function(entity,config){var self=this;var canSelectRows=config.canSelectRows;this.entity=ko.isObservable(entity)?entity:ko.observable(entity);if(this.entity()['__kg_selected__']===undefined){this.entity()['__kg_selected__']=ko.observable(false);}
...
var element=event.target;if(element.type=="checkbox"&&element.parentElement.className.indexOf("kgSelectionCell"!==-1)){return true;}else{if(self.selected()){self.selected(false);}else{self.selected(true);}}
...
self.sortInfo({column:col,direction:direction});};var sortData=function(){var data=dataSource(),sortInfo=self.sortInfo(),col,direction,sortFn,item,propPath,prop,i;if(!data||!sortInfo||options.useExternalSorting){internalSortedData(data);return;}
...
propAEmpty=isEmpty(propA);propBEmpty=isEmpty(propB);if(propAEmpty&&propBEmpty){return 0;}else if(propAEmpty){return 1;}else if(propBEmpty){return-1;}
...
return grid;};this.clearGridCache=function(){self.gridCache={};};this.assignGridEventHandlers=function(grid){grid.$viewport.scroll(function(e){var scrollLeft=e.target.scrollLeft,scrollTop=e.target.scrollTop;grid.adjustScrollLeft(scrollLeft);if(scrollTimer){clearTimeout(scrollTimer);}
scrollTimer=setTimeout(function(){scrollTimer=null;grid.adjustScrollTop(scrollTop);},200);});var $parent=grid.$root.parent();if($parent.length==0){$parent=grid.$root;}
...
this.gridId="kg"+kg.utils.newId();this.initPhase=0;if(this.config.footerRowHeight===defaults.footerRowHeight&&(!this.config.canSelectRows||!this.config.isMultiSelect)){defaults.footerRowHeight=30;this.config.footerRowHeight=30;}
